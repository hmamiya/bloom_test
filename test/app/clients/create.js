...
xdescribe('client/create', () => { 
    ...
    xdescribe('企業登録_正常系', () => {
        ...
        it('企業名が表示されること', () => {
            var client_name_confirm = browser.getText('//tbody/tr[1]/td[2]');
            expect(client_name_confirm).toEqual("株式会社" + client_name.value);
        });
        it('業種(10文字)が表示されること', () => {
            var industry_confirm = browser.getText('//tbody/tr[1]/td[4]');
            expect(industry_confirm).toEqual("人材サービス・アウト...")
        });
        it('住所が表示されること', () => {
            var address_confirm = browser.getText('//tbody/tr[1]/td[5]');
            expect(address_confirm).toEqual("東京都千代田区" + address2.value)
        });
    });

    //異常系（必須）
    describe('企業登録_異常系_必須', () => {
        it('入力せずに送信', () => {
            //企業一覧画面に遷移
            browser.url('http://54.95.18.112:8084/client');
